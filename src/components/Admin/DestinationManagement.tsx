...
export function DestinationManagement() {
  ...
  const handleEdit = (destination: Destination) => {
    setFormData({
      ...
      flag_image: destination.flag_image || '',
    ...
    try {
      ...
      if (data.content) {
        setFormData(prev => ({
          ...
          fun_facts: data.content.fun_facts || prev.fun_facts,
  ...
  const handleSave = async () => {
    ...
    try {
      const destinationData = {
        ...
        highlights: formData.highlights.filter(h => h.title.trim()).map(h => ({ title: h.title, description: h.description, image: h.image || '' })),
        regions: formData.regions.filter(r => r.name.trim()).map(r => ({ name: r.name, description: r.description })),
