...
export function DomainSettings() {
  ...
  const handleCreateSubdomain = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-auto-subdomain`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Subdomain aanmaken mislukt');
      }

      if (result.already_exists) {
        setSuccess(`Je hebt al een subdomain: ${result.domain}`);
      } else {
        setSuccess(`Subdomain succesvol aangemaakt: ${result.domain}`);
      }

      await loadDomains();
    } catch (err: any) {
      console.error('Error creating subdomain:', err);
      setError('Fout bij aanmaken subdomain: ' + err.message);
    } finally {
      setCreatingSubdomain(false);
    }
  };

  const handleDeleteDomain = async (domainId: string, isAutoGenerated: boolean) => {
    if (isAutoGenerated) {
      if (!confirm('Weet je zeker dat je je gratis subdomain wilt verwijderen? Je kunt later altijd een nieuw subdomain aanmaken.')) return;
    } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gratis subdomain of koppel je eigen domein</p>
        </div>
        <div className="flex space-x-3">
          {!domains.some(d => d.domain_type === 'subdomain') && (
            <button
            ...
            >
              ...
              <span>{creatingSubdomain ? 'Bezig...' : 'Gratis Subdomain'}</span>
            </button>
          )}
          <button
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">Start met een gratis subdomain of koppel je eigen domein</p>
            <button
              onClick={handleCreateSubdomain}
              disabled={creatingSubdomain}
              className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50"
            >
              {creatingSubdomain ? 'Bezig...' : 'Maak Gratis Subdomain'}
            </button>
          </div>
        ) : (
          domains.map((domain) => (
            <div key={domain.id} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {domain.domain_type === 'subdomain' && domain.status === 'verified' && (
                    <div className="mt-4">
                      <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                        <p className="text-sm text-green-700 flex items-center gap-2">
                          <Check className="w-4 h-4" />
                          <span>
                            <strong>Klaar voor gebruik!</strong> Je gratis subdomain is actief en direct beschikbaar. Geen DNS configuratie nodig.
                          </span>
                        </p>
                        <a
                          href={`https://${domain.domain}/home`}
                          target="_blank"
                  ...
                  {domain.domain_type === 'custom' && domain.status === 'pending' && (
                  ...
                  {domain.domain_type === 'custom' && domain.status === 'verified' && domain.dns_verified_at && (
                    <div className="mt-4 space-y-4">
                ...
                <div className="flex space-x-2 ml-4">
                  ...
                  {!domain.auto_generated && (
                    <button
                      onClick={() => handleDeleteDomain(domain.id, domain.auto_generated)}
