...
export function WordPressNewsManager() {
  ...
  const loadBrandInfo = async () => {
    ...
    try {
      ...
      if (!brandError && brandData?.wordpress_url) {
        setWordpressUrl(brandData.wordpress_url);
      }
    } catch (error) {
  ...
  const loadWordPressArticles = async () => {
    if (!effectiveBrandId || !wordpressUrl) return;

    try {
      const { data, error } = await supabase.functions.invoke('wordpress-news', {
        body: {
          action: 'list',
  ...
  const handleEditInWordPress = (assignment: NewsAssignment) => {
    if (!wordpressUrl) {
      alert('WordPress URL niet geconfigureerd');
      return;
    }

    const wpEditUrl = `${wordpressUrl}/wp-admin/post.php?post=${assignment.news_item.id}&action=edit`;
  ...
  const createNewArticle = () => {
    ...
    const wpNewPostUrl = `${wordpressUrl}/wp-admin/post-new.php`;
    window.open(wpNewPostUrl, '_blank');
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">WordPress Nieuws</h2>
            <p className="text-sm text-gray-500">Beheer je WordPress artikelen</p>
          </div>
        </div>
        {activeTab === 'own' && (
          <button
      ...
      {displayedAssignments.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">
            {activeTab === 'assigned' ? 'Geen toegewezen nieuwsberichten' : 'Geen WordPress artikelen'}
          </h3>
          <p className="mt-1 text-sm text-gray-500">
            {activeTab === 'assigned'
              ? 'Er zijn nog geen nieuwsberichten toegewezen aan jouw brand.'
              : 'Maak je eerste artikel in WordPress met de knop hierboven.'}
          </p>
          {activeTab === 'own' && (
            <button
              onClick={createNewArticle}
              ...
              <tr key={assignment.id} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${
                    ...
                    assignment.status === 'brand' ? 'bg-blue-100 text-blue-700' :
                    'bg-orange-100 text-orange-700'
                  }`}>
                    {assignment.status === 'mandatory' ? 'Verplicht' :
                     assignment.status === 'brand' ? 'WordPress' :
                     'Optioneel'}
                  </span>
                </td>
                ...
                <td className="px-6 py-4 text-center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {assignment.status !== 'mandatory' && (
                      <>
                        <button
                          onClick={() => handleEditInWordPress(assignment)}
